<template lang="pug">
  div
    span TODO:
    input.input(type="text" v-model="state.text" placeholder="100文字まで")
    div
      input#color1(type="radio" name="color" value="#80ffff" checked)
      label(for="color1") シアン
      input#color2(type="radio" name="color" value="#ff80ff")
      label(for="color2") マゼンタ
      input#color3(type="radio" name="color" value="#ffff80")
      label(for="color3") イエロー
    div(v-if="!state.isError")
      button(
        style="width:100%"
        v-on:click="onAdd"
        v-bind:disabled="(state.text.length == 0) || state.isError"
      )
        | 登録
    div(v-if="state.isError")
      span.error 入力できる文字は100文字までです
</template>
